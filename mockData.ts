...
const generatePlayers = (count: number): Player[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const isInterested = i < 5; // Prepopulate the first 5 players as "Interested"
    ...
    players.push({
      ...
      // Pre-calculate fitScore for interested players (10-90 range)
      fitScore: isInterested ? Math.floor(Math.random() * 81) + 10 : undefined
    });
  }
